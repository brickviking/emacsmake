...
#+PROPERTY: header-args :tangle /home/viking/src/bash/emacsmake/emacsmake

* Header and Changelog
A few notes made about things added/changed, along with a long-term target to meet for
emacsen versions earlier than 28.0.50. Heck, 18.x (and earlier beasts) were done on far
wimpier machines and earlier versions (13-18.58) predated the autoconf software
suite which wasn't released until 1991.
...
Anyhow, there are some other tweakable values here which make sense except perhaps the playpen, which
ends up being my own personal settings testbed, so I don't blow away my main configuration.
...
Pretty obvious what this does, though it does presume we're in the correct directory,
...
** Make command
Only note here is that we force a full recompile so we don't accidentally install stale
