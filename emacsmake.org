...
Something's weird with the cp, timestamp doesn't seem to get updated. Found out why, fixed.

This is an idea to include into emacsmake at the install stage, as it'll eventually become a
way to find out what I already compiled, if I compiled it. We start off with a default of gtk3
in WMGR, which only changes if we find something specific.
...
whatMadeMe() {
    ...
    if [[ -f config.log ]]; then
        mapfile -t TOOLKIT < <(sed -n '7p' config.log | cut -c17-)
