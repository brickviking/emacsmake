...
EMACSCONFIGPARAMS=(
    ...
    "--prefix=${EMACSHOME}" )
# Default X toolkit if we don't change it with a -c{g2,l,m} option
WMGR=gtk3
...
# You'll need this if the autogen.sh hasn't been run to get us a configure script
# We should also make sure we run this in exactly the right place.
runAutogen() {
    ./autogen.sh
}
#+END_SRC
