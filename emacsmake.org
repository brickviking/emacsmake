...
First off, you'll need to install the source code to Emacs, adjust =COMPILEHOME=. You can either
...
if [[ -n $2 ]]; then # In short, if 2 or more args
    ...
    case $1 in 
        "-h"|"--help"|"-?") helpMe ;;
        "-d") pushd "${COMPILEHOME}" >/dev/null
              cleanMe
              popd >/dev/null ;;
	"-D") pushd "${COMPILEHOME}" >/dev/null
              cleanMeGood
              popd >/dev/null ;;
        "-a") pushd "${COMPILEHOME}" >/dev/null
              execMakeMe
              popd >/dev/null ;;
        "-e") pushd "${COMPILEHOME}" >/dev/null # Eventually changes to ${EMACSHOME}
              execMe
              popd >/dev/null ;;
        "-c") pushd "${COMPILEHOME}" >/dev/null
              configMe
              popd >/dev/null ;; # Uses GTK3 with xwidgets
        "-cg") pushd "${COMPILEHOME}" >/dev/null
              configMeGTK
              popd >/dev/null ;;
        "-cg2") pushd "${COMPILEHOME}" >/dev/null
              configMeGTK2
              popd >/dev/null ;;
        "-cl") pushd "${COMPILEHOME}" >/dev/null
              configMeLucid
              popd >/dev/null ;;
        "-cm") pushd "${COMPILEHOME}" >/dev/null
              configMeMotif
              popd >/dev/null ;;
        "-m") pushd "${COMPILEHOME}" >/dev/null
              makeMe
              popd >/dev/null ;;
        "-i") pushd "${COMPILEHOME}" >/dev/null
              installMe
              popd >/dev/null ;;
        ...
        "-r") pushd "${EMACSHOME}">/dev/null
              runMe
              popd >/dev/null ;;
	"-u") pushd "${COMPILEHOME}" >/dev/null
              uninstallMe
              popd >/dev/null ;;
        # Find out what we have before
        "-w") pushd "${COMPILEHOME}" >/dev/null
              whatMadeMe
              popd >/dev/null ;;
        ,*) pushd "."
              helpMe ;;
